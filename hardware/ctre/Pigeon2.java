...
public class Pigeon2 implements LoggableHardware, AutoCloseable {
  ...
  private SimDouble m_simPigeonYaw;
  ...
  public Pigeon2(ID id) {
    ...
    this.m_simPigeonYaw = new SimDouble(SimDeviceDataJNI.getSimValueHandle(SimDeviceDataJNI.getSimDeviceHandle("Pigeon_Sensor[0]"), "Yaw"));
  ...
  private void updateInputs() {
    ...
    m_inputs.yawAngle = Units.Degrees.of(getAngle() * -1);
  ...
  public StatusCode setStatusFramePeriod(PigeonStatusFrame statusFrame, int frequencyHz) {
    switch (statusFrame) {
      ...
      case YAW_RATE:
        return m_pigeon.getAngularVelocityZWorld().setUpdateFrequency(frequencyHz);
      default:
  ...
  public void reset() {
    m_pigeon.reset();
    yawOffset = m_pigeon.getYaw().getValueAsDouble();
    m_simPigeonYaw.set(0.0);
  }
