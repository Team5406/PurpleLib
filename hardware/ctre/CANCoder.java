...
public class CANCoder implements LoggableHardware, AutoCloseable {
 ...
 /**
  ...
  public StatusCode configFactoryDefault() {
    return configFactoryDefault(true);
  }

  
/**
   * Configures all persistent settings to defaults (overloaded so timeoutMs is 50 ms).
   * @param resetOffset specifies whether to reset the absolute magnetic offset (false = don't reset)
   *
   * @return Status Code generated by function. 0 indicates no error.
   */
  public StatusCode configFactoryDefault(boolean resetOffset) {

    CANcoderConfiguration config = new CANcoderConfiguration();

    if(!resetOffset){
      CANcoderConfigurator canCoderConfigurator = m_canCoder.getConfigurator();
